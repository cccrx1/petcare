...
**⚠️ 重要**：在配置Dify智能体和工作流之前，请务必阅读输出格式规范文档：

📄 **[DIFY_OUTPUT_FORMAT.md](./DIFY_OUTPUT_FORMAT.md)**
...
**请严格按照输出格式规范配置智能体和工作流的输出**，否则系统将无法正确解析和处理返回的数据。

## 注意事项

1. **输出格式**：⚠️ 务必按照 [DIFY_OUTPUT_FORMAT.md](./DIFY_OUTPUT_FORMAT.md) 中的格式要求配置智能体和工作流输出
2. **安全性**：配置文件中包含敏感信息（API密钥、数据库密码等），请勿提交到版本控制系统
3. **环境变量**：建议使用环境变量来存储敏感信息，而非直接写在配置文件中
4. **配置验证**：在修改配置后，建议进行配置验证，确保格式正确
5. **备份**：定期备份配置文件，以便在需要时快速恢复
